    ...
    <>
      <section className="landing-hero">
        <div className="container landing-hero-grid">
          <div>
            <span className="eyebrow landing-eyebrow-gold">
              Accra's most booked beauty studio
            </span>
            <h1>
              Look extraordinary. <em>Feel royal.</em>
            </h1>
            <p className="landing-hero-copy">
              From sharp fades to fresh lashes, gel pedicures to full glam —
              Purple brings every beauty ritual under one roof, booked in under
              a minute.
            </p>
            <div className="landing-hero-actions">
              ...
              <Link to="/services" className="btn-ghost on-dark">
                Explore services
              </Link>
            </div>
          </div>
      ...
      <section className="section">
        ...
        {services.length === 0 ? (
          <p className="empty-text">
            Loading services… (make sure the Django API is running)
          </p>
        ) : (
            ...
            {services.map((s) => (
              ...
              >
                ...
                <div className="service-tile-body">
                  ...
                  <h3>{s.name}</h3>
                  <p>
                    GHS {s.price} · {s.duration_minutes} min
                  </p>
                </div>
              </Link>
            ))}
          ...
          <div className="about-grid">
            ...
            <div>
              ...
              <h2 className="about-heading">
                Built for Accra, one chair at a time.
              </h2>
              <p className="about-copy">
                Purple started as a single studio in East Legon with one
                promise: no more waiting rooms, no more phone tag. Every
                stylist, lash tech, and nail artist on our platform is vetted
                for craft and consistency.
              </p>
              <p className="about-copy">
                Today we connect thousands of clients across Accra with the
                right specialist for their hair, skin, and nails — booked in
                minutes, not calls.
              </p>
            </div>
          </div>
      ...
      <section>
        ...
        <div className="section locations-section">
          ...
          {salons.length === 0 ? (
            <p className="empty-text">
              No studio locations available right now.
            </p>
          ) : (
            <div className="locations-grid">
              {salons.map((s) => (
                <div key={s.id} className="card location-card">
                  ...
                  <p>{s.address}</p>
                  <p>
                    {s.city}, {s.region.replace("_", " ")}
                  </p>
                  <p>
                    Open {s.opens_at?.slice(0, 5)} – {s.closes_at?.slice(0, 5)}
                  </p>
                  {s.phone && <p className="location-phone">{s.phone}</p>}
                </div>
              ))}
            </div>
          )}
